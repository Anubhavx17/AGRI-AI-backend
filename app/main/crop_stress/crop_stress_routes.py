...
def create_result_entry(user_id, tiff_local_path, excel_local_path,tiff_min_max,redsi_min_max_str, selected_date, selected_parameter, geojson,project_id,temp_df):
...
def main():
    ...
    data = request.get_json()
    user_id = get_jwt_identity()
    geojson_data, input_date, crop, selected_parameter = get_input_data(data) ## tweaking the input data
    project_id = data.get('project_id')

    # if result already present then return
    existing_result_response = result_already_exists(data.get('date'), selected_parameter, project_id, user_id)
        
    if existing_result_response:
            return existing_result_response  # Return if result already exists
    ...
    ## generating the graph related stuff which is farm wise
...
def get_input_data(data):
    ...
    return geojson_data,input_date, crop,selected_parameter
