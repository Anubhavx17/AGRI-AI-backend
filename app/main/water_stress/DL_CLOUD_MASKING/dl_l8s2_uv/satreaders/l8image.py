...
import numpy as np
import rasterio
import app.main.water_stress.DL_CLOUD_MASKING.dl_l8s2_uv.utils as utils
...
def compute_toa_single(img, band, metadata, sun_elevation_correction=True):
    ...
    if band < 10:
        dictio_rescaling = metadata["LEVEL1_RADIOMETRIC_RESCALING"]
...
class L8Image:
    ...
    def __init__(self, folder_tiffs, slice_rows_cols=None):
        ...
        dictio_metadata = self.metadata["PROJECTION_ATTRIBUTES"]
        ...
        proj_param = self.metadata["PROJECTION_ATTRIBUTES"]
        ...
        pm = self.metadata["PROJECTION_ATTRIBUTES"]
        ...
        temporal_dict = self.metadata["IMAGE_ATTRIBUTES"]
        self.start_date = datetime.strptime(temporal_dict["DATE_ACQUIRED"] + " " +
                                            temporal_dict["SCENE_CENTER_TIME"][:8],
    ...
    def load_band(self, band, compute_toa_flag=False,
